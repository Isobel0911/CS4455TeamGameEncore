...
public class ThirdPersonCamera : MonoBehaviour {
    ...
    private bool cameraRotatable = true;
    private Quaternion fixedRotation;

    private Transform nowParent;
    private Transform nowOriginal;
    ...
    void Update() {
        ...
        if (Input.GetMouseButton(2)) {
            ...
            targToCamNoY = nowParent.position - transform.position;
            yCompOrg = targToCamNoY.y;
            targToCamNoY.y = 0;

            yAngle = Vector3.Angle(targToCamNoY, nowParent.position - transform.position);
        ...
        playerToCamera = transform.position - nowParent.position;
        if (cameraRotatable) {
            ...
            transform.rotation = Quaternion.LookRotation(nowParent.position - transform.position, Vector3.up);
        }
    }

    public void disableCameraRotate() {
        cameraRotatable = false;
        nowParent = parentTransform;
        nowOriginal = originalTransform;
    }
    public void enableCameraRotate()  {
        cameraRotatable = true;
    }
    ...
    void FixedUpdate() {
        ...
        RaycastHit hitInfo;
        playerToCamera = transform.position - nowParent.position;
        Vector3 desiredPosition = nowOriginal.position; // Make sure to have a variable to store the camera's original position

        // Check for any obstructions between the parentTransform and the camera
        if (Physics.Raycast(nowParent.position, playerToCamera.normalized, out hitInfo, playerToCamera.magnitude)) {
