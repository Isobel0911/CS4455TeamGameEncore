...
public class ThirdPersonCamera : MonoBehaviour {
	...
	void LateUpdate ()
	{
        ...
        if (desiredPose != null)
        {
            ...
            targForward = (target.position - this.transform.position).normalized;

            transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(targForward, Vector3.up), rotationSmoothTime * Time.deltaTime);
        }
    }
}
